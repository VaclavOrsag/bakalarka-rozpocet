...
\begin{itemize}
    \item \textbf{Plánování:} Uživatel může nastavit měsíční rozpočet pro každou kategorii v účetní osnově.
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Detail měsíce:} Z dashboardu musí být možný přístup do detailu konkrétního měsíce, kde uživatel uvidí rozpad výdajů dle kategorií.
\end{itemize}
...
Pro vývoj aplikace byl zvolen jazyk \textbf{Python} verze 3.x. Jeho hlavní výhodou je rozsáhlá standardní knihovna a silná podpora pro práci s daty.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \texttt{home\_tab.py} – Úvodní obrazovka záložka "Home", která provází uživatele tutoriálem. Po jeho dokončení se změní na Dashboard.
            \item \texttt{dashboard\_tab.py} – Implementace logiky Dashboardu (přehled měsíčního plnění), který je součástí Home tabu.
            \item \texttt{sources\_tab.py} – Záložka "Transakce". Umožňuje správu, filtrování a import historických i aktuálních transakcí.
            \item \texttt{accounting\_structure\_tab.py} – Záložka "Účetní osnova". Slouží k tvorbě a úpravě účetní osnovy.
            \item \texttt{budget\_tab.py} – Záložka "Rozpočet". Umožňuje nastavovat finanční limity (rozpočty) pro jednotlivé kategorie.
            \item \texttt{analysis\_tab.py} – Záložka "Analýza". Poskytuje dynamický pohled na data formou kontingenční tabulky (pivot table).
        \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Každá transakce je navázána na jednu kategorii (vazba N:1).(Po přiřazení kategorie do účetní osnovy.)
...
\begin{enumerate}
    \item Při importu se zkontroluje, zda hodnota ve sloupci \textbf{Co} odpovídá existující kategorii v tabulce \texttt{categories}.
    ...
    \item Pokud kategorie neexistuje, vytvoří se v záložce \textbf{Účetní osnova} nová kategorie typu \textbf{Leaf} s názvem odpovídajícím hodnotě ve sloupci \textbf{Co}. Tuto kategorii lze zařadit do účetní osnovy.
\end{enumerate}

\subsubsection{Výpočet hierarchie (Roll-up)}
Pro zobrazení dat v záložce Analýza a Rozpočet je nutné sčítat hodnoty z podkategorií do nadřazených kategorií. Tento proces (roll-up) je implementován rekurzivně.
Každá kategorie může být buď \textit{LEAF} (listová, obsahuje přímo transakce) nebo \textit{CUSTOM} (kontejner, obsahuje jiné kategorie). Hodnota kontejneru je vždy součtem hodnot jeho dětí. Tento výpočet probíhá v reálném čase při načítání dat pro zobrazení, aby byla zajištěna konzistence.
