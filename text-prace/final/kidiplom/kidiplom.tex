...
jednotlivé měsíce, které zobrazujeme jako tlačítka pro rychlý přístup k datům daného měsíce. Jednotlivé tlačítka měsíců jsou barevně odlišena podle toho, zda překračují očekávané plnění rozpočtu ((měsíc/12)*100) nebo ne.: 
Pokud je měsíc v limitu má \textbf{zelenou barvu}, pokud překračuje limit, ale je v rozmezí 5\% má \textbf{žlutou barvu} pokud překračuje více má \textbf{červenou barvu}.
\begin{figure}[H]
    \centering
